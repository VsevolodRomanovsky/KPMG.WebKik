...
@Component({
    template: require('./register4.component.html'),
    providers: []
})
export class Register4Component implements OnInit {
    model: Register4ViewModel;
    projectId: number;
    companyId: number;
    shareId: number;
    registerId: number;
    ...
    constructor(private route: ActivatedRoute, private router: Router, private registerService: Register4Service) { }
    ...
    ngOnInit(): void {
        ...
        if (this.registerId == 0) {
            this.model = new Register4ViewModel();
            this.model.Year = this.year;
            this.model.OwnerProjectCompanyId = this.shareId;
            this.model.Type = RegisterType.Register4;
            this.isNew = true;
